...
export default function MissionValueEngine() {
  ...
  return (
    <div className="bg-[#141517] border border-white/5 rounded-2xl p-5 flex flex-col justify-between h-full font-sans shadow-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 gap-3.5 relative items-center mb-6">
        {steps.map((step, idx) => {
          return (
      ...
      <div className="border-t border-white/5 pt-4 grid grid-cols-1 md:grid-cols-12 gap-5 items-center">
        ...
        <div className="md:col-span-4 flex flex-col items-center justify-center py-2 bg-gradient-to-tr from-[#251613] to-[#141517] border border-[#f0522f]/25 rounded-2xl shadow-md">
          <span className="text-[#f0522f] text-[9px] uppercase font-extrabold tracking-widest leading-none mb-1">
            Mission Value Score
          </span>
          <span className="text-white text-3xl font-black font-mono leading-none tracking-tight drop-shadow-[0_0_10px_rgba(240,82,47,0.35)]">
            92
          </span>
          <span className="text-[#6b7280] text-[8px] font-semibold mt-1">
        ...
        {/* Weights bars (Cols 4) */}
        <div className="md:col-span-4 flex flex-col gap-1.5 pl-0 md:pl-2">
              ...
              return (
                <div key={w.name} className="flex flex-col gap-0.5">
                  ...
                  <div className="w-full bg-[#222428] rounded-full h-1">
                    <div
                      className={`h-1 rounded-full ${w.color}`}
                      style={{ width: `${w.pct}%` }}
                    ></div>
                  </div>
                </div>
